defmodule JetLagServer.Games.GameSettings do
  ...
  def changeset(game_settings, attrs) do
    game_settings
    |> cast(attrs, [:units, :hiding_zones, :hiding_zone_size, :game_duration, :day_start_time, :day_end_time])
    |> validate_required([:units, :hiding_zones, :hiding_zone_size, :game_duration, :day_start_time, :day_end_time])
    |> validate_inclusion(:units, ["ansi", "iso"])
    |> validate_subset(:hiding_zones, ["bus_stops", "local_trains", "regional_trains", "high_speed_trains", "subways", "tram"])
    |> validate_number(:hiding_zone_size, greater_than_or_equal_to: 100, less_than_or_equal_to: 2000)
